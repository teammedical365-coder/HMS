...
import { adminAPI, uploadAPI, hospitalAPI, aiWalletAPI, policyAPI } from '../../utils/api';
...
const HospitalAdminDashboard = () => {
    ...
    // --- Hospital Policy Management State ---
    const [policies, setPolicies] = useState([]);
    const [policyStats, setPolicyStats] = useState({ total: 0, active: 0, mandatory: 0 });
    ...
    // --- Hospital Branding & Communication State ---
    ...
    const [emailPreviewModal, setEmailPreviewModal] = useState({ open: false, type: 'LOGIN_OTP', preview: null });
    ...
    // Fetch data when switching to inventory, lab pricing, accounts, AI tracking, policies, or branding tabs
    useEffect(() => {
    ...
    const fetchPolicies = async () => {
        try {
            ...
            if (res.success) {
                ...
                if (res.stats) setPolicyStats(res.stats);
            }
        } catch (err) {
            console.error('Failed to fetch policies:', err);
            toast.error('Failed to load hospital policies');
        } finally {
    ...
    const handleSavePolicy = async (e) => {
        ...
        try {
            ...
            if (policyModal.mode === 'create') {
                const res = await policyAPI.createPolicy(policyForm);
                if (res.success) {
                    toast.success('Policy created successfully');
                    setPolicyModal({ open: false, mode: 'create', policy: null });
                    fetchPolicies();
                }
            } else if (policyModal.mode === 'edit') {
                const res = await policyAPI.updatePolicy(policyModal.policy._id, policyForm);
                if (res.success) {
                    toast.success('Policy updated successfully (version auto-incremented)');
                    setPolicyModal({ open: false, mode: 'create', policy: null });
                    fetchPolicies();
                }
            }
    ...
    const handleTogglePolicyStatus = async (policy) => {
        try {
            const nextStatus = !policy.isActive;
            const res = await policyAPI.togglePolicyStatus(policy._id, nextStatus);
            if (res.success) {
                toast.success(`Policy ${nextStatus ? 'activated' : 'deactivated'}`);
                setPolicies(prev => prev.map(p => p._id === policy._id ? { ...p, isActive: nextStatus } : p));
                setPolicyStats(prev => ({
                    ...prev,
    ...
    const handleDeletePolicy = async (policyId) => {
        confirmToast('Are you sure you want to delete this policy permanently?', async () => {
            try {
    ...
    const handleSeedDefaults = async () => {
        confirmToast('Seed default NABH-compliant hospital policy templates? This will add standard policies if they do not exist.', async () => {
            try {
                setSeedingPolicies(true);
                const res = await policyAPI.seedDefaults();
                if (res.success) {
                    toast.success(res.message || 'Standard policies seeded successfully');
                    fetchPolicies();
                }
            } catch (err) {
    ...
    const fetchBranding = async () => {
        try {
            ...
            const res = await hospitalAPI.getMyBranding();
            if (res.success && res.branding) {
                setBrandingData({
                    emailDisplayName: res.branding.emailDisplayName || '',
                    customHospitalName: res.branding.customHospitalName || '',
                    logoUrl: res.branding.logoUrl || '',
                    supportEmail: res.branding.supportEmail || '',
                    supportPhone: res.branding.supportPhone || '',
                    address: res.branding.address || ''
                });
            }
        } catch (err) {
    ...
    const handleSaveBranding = async (e) => {
        ...
        try {
            ...
            const res = await hospitalAPI.updateMyBranding(brandingData);
            if (res.success) {
                toast.success('Branding & communication settings saved successfully!');
            }
        } catch (err) {
    ...
    const handlePreviewEmail = async (type) => {
        try {
            ...
            const res = await hospitalAPI.previewEmail(type);
            if (res.success && res.preview) {
                setEmailPreviewModal({ open: true, type, preview: res.preview });
            }
        } catch (err) {
    ...
    const tabs = [
        ...
        { id: 'policies', label: 'Hospital Policies' },
        { id: 'branding', label: 'Branding & Communication' },
    ];
    ...
    return (
        <div className="hospitaladmin-page">
            <div className="hospitaladmin-container">
                ...
                {/* HOSPITAL POLICIES TAB */}
                {activeTab === 'policies' && (
                    <div className="tab-pane">
                        <div className="ha-policies-header-container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
                            <div>
                                <h2 style={{ fontSize: '1.4rem', fontWeight: 800, color: '#0f172a', margin: '0 0 6px 0', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    <span>📜</span> Hospital Policies & NABH Compliance
                                </h2>
                                <p style={{ margin: 0, color: '#64748b', fontSize: '0.9rem' }}>
                                    Configure hospital-specific terms, consent provisions, admission rules, and patient rights.
                                </p>
                            </div>
                            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                                ...
                                >
                                    <span>⚡</span> {seedingPolicies ? 'Seeding...' : 'Seed Standard Policies (NABH)'}
                                </button>
                                <button
                                    type="button"
                                    onClick={() => {
                                        setPolicyForm({
                                            ...
                                            isActive: true,
                                            applicableTo: 'ALL',
                                            displayOrder: (policies.length || 0) + 1
                                        });
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px' }}>
                            <div className="admin-card" style={{ padding: '18px 22px', display: 'flex', alignItems: 'center', gap: '16px' }}>
                                <div style={{ width: '46px', height: '46px', borderRadius: '12px', background: '#eff6ff', color: '#2563eb', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.4rem' }}>📋</div>
                                <div>
                                    <div style={{ fontSize: '1.5rem', fontWeight: 800, color: '#1e293b' }}>{policyStats.total}</div>
                                    <div style={{ fontSize: '0.82rem', color: '#64748b', fontWeight: 600 }}>Total Policies</div>
                                </div>
                            </div>
                            <div className="admin-card" style={{ padding: '18px 22px', display: 'flex', alignItems: 'center', gap: '16px' }}>
                                <div style={{ width: '46px', height: '46px', borderRadius: '12px', background: '#ecfdf5', color: '#059669', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.4rem' }}>✅</div>
                                <div>
                                    <div style={{ fontSize: '1.5rem', fontWeight: 800, color: '#1e293b' }}>{policyStats.active}</div>
                                    <div style={{ fontSize: '0.82rem', color: '#64748b', fontWeight: 600 }}>Active in Patient Portals</div>
                                </div>
                            </div>
                            <div className="admin-card" style={{ padding: '18px 22px', display: 'flex', alignItems: 'center', gap: '16px' }}>
                                <div style={{ width: '46px', height: '46px', borderRadius: '12px', background: '#fef2f2', color: '#dc2626', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.4rem' }}>🔒</div>
                                <div>
                                    <div style={{ fontSize: '1.5rem', fontWeight: 800, color: '#1e293b' }}>{policyStats.mandatory}</div>
                                    <div style={{ fontSize: '0.82rem', color: '#64748b', fontWeight: 600 }}>Mandatory Agreements</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '12px', marginBottom: '20px' }}>
                            {['ALL', 'General', 'Admission', 'Discharge', 'Billing & Refund', 'OPD Consultation', 'Emergency Care', 'Visitor & Attendant', 'Patient Rights & Responsibilities', 'Data Privacy & Consent', 'Pharmacy & Medication'].map(cat => (
                                <button
                                    key={cat}
                                    type="button"
                                    ...
                                    style={{
                                        ...
                                        borderRadius: '20px',
                                        border: '1px solid',
                                        borderColor: policyCategoryFilter === cat ? '#2563eb' : '#e2e8f0',
                                        background: policyCategoryFilter === cat ? '#2563eb' : '#ffffff',
                                        color: policyCategoryFilter === cat ? '#ffffff' : '#64748b',
                                        fontWeight: 600,
                                        fontSize: '0.82rem',
                                        cursor: 'pointer',
                                        whiteSpace: 'nowrap',
                        ...
                        {loadingPolicies ? (
                            ...
                            (() => {
                                const filteredPolicies = policies.filter(p => policyCategoryFilter === 'ALL' || p.category === policyCategoryFilter);
                                if (filteredPolicies.length === 0) {
                                    return (
                                        <div className="admin-card" style={{ padding: '50px 20px', textAlign: 'center' }}>
                                            <div style={{ fontSize: '3rem', marginBottom: '14px' }}>📄</div>
                                            <h3 style={{ color: '#1e293b', marginBottom: '6px' }}>No Policies Found</h3>
                                            <p style={{ color: '#64748b', maxWidth: '460px', margin: '0 auto 20px auto', fontSize: '0.9rem' }}>
                                                {policyCategoryFilter !== 'ALL' ? `No policies configured under "${policyCategoryFilter}".` : 'No hospital policies have been configured yet.'}
                                            </p>
                                            {policies.length === 0 && (
                                                <button
                                                    type="button"
                                ...
                                return (
                                    <div style={{ display: 'grid', gap: '14px' }}>
                                        {filteredPolicies.map((p) => (
                                            <div key={p._id} className="admin-card" style={{ padding: '18px 22px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '14px' }}>
                                                <div style={{ flex: '1 1 400px' }}>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px', flexWrap: 'wrap' }}>
                                                        <span style={{ fontWeight: 700, fontSize: '1.02rem', color: '#1e293b' }}>{p.title}</span>
                                                        <span style={{ fontSize: '0.74rem', padding: '2px 8px', borderRadius: '12px', background: '#dbeafe', color: '#1e40af', fontWeight: 700 }}>
                                                            v{p.version}
                                                        </span>
                                                        ...
                                                        {p.isMandatory && (
                                                            <span style={{ fontSize: '0.72rem', padding: '2px 8px', borderRadius: '12px', background: '#fee2e2', color: '#b91c1c', fontWeight: 700 }}>
                                                                MANDATORY
                                                            </span>
                                                        )}
                                                        <span style={{ fontSize: '0.72rem', padding: '2px 8px', borderRadius: '12px', background: '#f8fafc', color: '#64748b', border: '1px solid #e2e8f0' }}>
                                                            {p.applicableTo}
                                                        </span>
                                                    </div>
                                                    <p style={{ margin: 0, color: '#64748b', fontSize: '0.86rem', lineHeight: 1.4, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                                                        {p.content}
                                                    </p>
                                                </div>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                                    <button
                                                        ...
                                                        style={{
                                                            ...
                                                            borderRadius: '6px',
                                                            border: '1px solid',
                                                            borderColor: p.isActive ? '#86efac' : '#cbd5e1',
                                                    ...
                                                    <button
                                                        type="button"
                                                        onClick={() => setPolicyModal({ open: true, mode: 'preview', policy: p })}
                                                        style={{ padding: '7px 14px', background: '#f1f5f9', color: '#334155', border: '1px solid #cbd5e1', borderRadius: '6px', fontWeight: 600, fontSize: '0.8rem', cursor: 'pointer' }}
                                                    >
                                                        Preview
                                                    </button>
                                                    <button
                                                        ...
                                                        style={{ padding: '7px 14px', background: '#eff6ff', color: '#2563eb', border: '1px solid #bfdbfe', borderRadius: '6px', fontWeight: 600, fontSize: '0.8rem', cursor: 'pointer' }}
                                                    >
                                                        Edit
                                                    </button>
                                                    <button
                                                        ...
                                                        style={{ padding: '7px 14px', background: '#fee2e2', color: '#dc2626', border: '1px solid #fecaca', borderRadius: '6px', fontWeight: 600, fontSize: '0.8rem', cursor: 'pointer' }}
                                                    >
                                                        Delete
                                                    </button>
                                                </div>
                ...
                {/* BRANDING & WHITE-LABEL COMMUNICATION TAB */}
                {activeTab === 'branding' && (
                    <div className="tab-pane">
                        <div className="ha-branding-header" style={{ marginBottom: '24px' }}>
                            <h2 style={{ fontSize: '1.4rem', fontWeight: 800, color: '#0f172a', margin: '0 0 6px 0', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <span>🏢</span> White-Label Communication & Branding
                            </h2>
                            <p style={{ margin: 0, color: '#64748b', fontSize: '0.9rem' }}>
                                Configure your hospital's dynamic identity for staff and patient emails, OTP dispatches, and appointment notifications.
                            </p>
                        </div>

                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(360px, 1fr))', gap: '24px', alignItems: 'start' }}>
                            {/* Left Card: Communication Branding Settings */}
                            <div className="admin-card" style={{ padding: '26px' }}>
                                <h3 style={{ fontSize: '1.1rem', fontWeight: 700, color: '#1e293b', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    <span>✉️</span> Sender & Identity Configuration
                                </h3>
                                ...
                                <form onSubmit={handleSaveBranding}>
                                    <div style={{ marginBottom: '18px' }}>
                                        ...
                                        <input
                                            type="text"
                                            value={brandingData.emailDisplayName}
                                            onChange={(e) => setBrandingData({ ...brandingData, emailDisplayName: e.target.value })}
                                            placeholder="e.g. Sunrise Multispeciality Hospital"
                                            style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '0.92rem', outline: 'none' }}
                                        />
                                        <div style={{ fontSize: '0.78rem', color: '#64748b', marginTop: '5px' }}>
                                            Staff OTP & patient notification emails will show: <br />
                                            <code style={{ background: '#f1f5f9', padding: '2px 6px', borderRadius: '4px', color: '#0369a1' }}>
                                                From: "{brandingData.emailDisplayName || hospitalInfo?.name || 'Hospital Name'}" &lt;noreply@medical365.in&gt;
                                            </code>
                                        </div>
                                    </div>
                                    ...
                                    <div style={{ marginBottom: '18px' }}>
                                        <label style={{ display: 'block', fontSize: '0.86rem', fontWeight: 700, color: '#334155', marginBottom: '6px' }}>
                                            Official / Custom Hospital Name
                                        </label>
                                        <input
                                            type="text"
                                            value={brandingData.customHospitalName}
                                            onChange={(e) => setBrandingData({ ...brandingData, customHospitalName: e.target.value })}
                                            placeholder={hospitalInfo?.name || "Official Hospital Name"}
                                            style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '0.92rem', outline: 'none' }}
                                        />
                                    </div>
                                    ...
                                    <div style={{ marginBottom: '18px' }}>
                                        ...
                                        <input
                                            ...
                                            placeholder="https://your-domain.com/hospital-logo.png"
                                            style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '0.92rem', outline: 'none' }}
                                        />
                                        {brandingData.logoUrl && (
                                            <div style={{ marginTop: '8px', padding: '8px', background: '#f8fafc', borderRadius: '8px', display: 'inline-block', border: '1px solid #e2e8f0' }}>
                                                <img src={brandingData.logoUrl} alt="Logo Preview" style={{ maxHeight: '40px', maxWidth: '160px', objectFit: 'contain' }} onError={(e) => { e.target.style.display = 'none'; }} />
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '18px' }}>
                                        <div>
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => setBrandingData({ ...brandingData, supportEmail: e.target.value })}
                                        ...
                                        <div>
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => setBrandingData({ ...brandingData, supportPhone: e.target.value })}
                                    ...
                                    <div style={{ marginBottom: '22px' }}>
                                        ...
                                        <textarea
                                            ...
                                            rows={2}
                                            style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '0.92rem', outline: 'none', resize: 'vertical' }}
                                        />
                                    </div>
                                    ...
                                    >
                                        {savingBranding ? 'Saving Settings...' : '✓ Save Branding Configuration'}
                                    </button>
                                </form>
                            </div>

                            {/* Right Card: Interactive Live Email Previews */}
                            <div className="admin-card" style={{ padding: '26px' }}>
                                <h3 style={{ fontSize: '1.1rem', fontWeight: 700, color: '#1e293b', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    <span>👁️</span> Live Email Verification & Preview
                                </h3>
                                <p style={{ color: '#64748b', fontSize: '0.86rem', lineHeight: 1.5, marginBottom: '20px' }}>
                                    Inspect the exact rendered email communication received by your hospital staff (Doctors, Reception, Nurses) and patients with active white-label headers.
                                </p>

                                <div style={{ display: 'grid', gap: '14px', marginBottom: '24px' }}>
                                    <div style={{ padding: '16px', borderRadius: '10px', background: '#f8fafc', border: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        <div>
                                            <div style={{ fontWeight: 700, color: '#1e293b', fontSize: '0.94rem' }}>Staff Login OTP Email</div>
                                            <div style={{ color: '#64748b', fontSize: '0.8rem' }}>Branded OTP dispatch sent to Doctors, Nurses & Staff</div>
                                        </div>
                                        <button
                                            type="button"
                                            onClick={() => handlePreviewEmail('LOGIN_OTP')}
                                            disabled={previewLoading}
                                            style={{ padding: '8px 16px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: '6px', fontWeight: 600, fontSize: '0.82rem', cursor: 'pointer' }}
                                        >
                                            Preview OTP
                                        </button>
                                    </div>
                                    ...
                                    <div style={{ padding: '16px', borderRadius: '10px', background: '#f8fafc', border: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        <div>
                                            <div style={{ fontWeight: 700, color: '#1e293b', fontSize: '0.94rem' }}>Appointment Confirmation Email</div>
                                            <div style={{ color: '#64748b', fontSize: '0.8rem' }}>Sent to patients upon clinic registration & booking</div>
                                        </div>
                                        <button
                                            type="button"
                                            onClick={() => handlePreviewEmail('APPOINTMENT_CONFIRMATION')}
                                            disabled={previewLoading}
                                            style={{ padding: '8px 16px', background: '#059669', color: '#fff', border: 'none', borderRadius: '6px', fontWeight: 600, fontSize: '0.82rem', cursor: 'pointer' }}
                                        >
                                            Preview Booking
                                        </button>
                                    </div>
                                ...
                                <div style={{ padding: '14px 16px', borderRadius: '8px', background: '#eff6ff', border: '1px solid #bfdbfe', fontSize: '0.82rem', color: '#1e40af', lineHeight: 1.5 }}>
                                    <strong>💡 White-Label Multi-Tenant Guarantee:</strong><br />
                                    All email dispatches from your hospital use your custom display name and tenant-specific logo and support details. Platform super-admins retain Medical365 branding.
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {policyModal.open && (policyModal.mode === 'create' || policyModal.mode === 'edit') && (
                    <div className="modal-overlay">
                        <div data-lenis-prevent="true" className="modal-content ha-modal-modern" style={{ maxWidth: '680px' }}>
                            <div className="ha-modal-header">
                                <div className="ha-modal-header-icon green">📜</div>
                                <div>
                                    <h3 className="ha-modal-title">{policyModal.mode === 'create' ? 'Add Hospital Policy' : 'Edit Hospital Policy'}</h3>
                                    <p className="ha-modal-subtitle">
                                        {policyModal.mode === 'create' ? 'Create a new hospital policy agreement or guideline.' : `Updating v${policyModal.policy?.version} — modifying title or text auto-increments version.`}
                                    </p>
                                </div>
                                <button type="button" onClick={() => setPolicyModal({ open: false, mode: 'create', policy: null })} className="ha-modal-close-btn" title="Close">✕</button>
                            </div>

                            <form onSubmit={handleSavePolicy} style={{ padding: '24px' }}>
                                <div className="form-group" style={{ marginBottom: '16px' }}>
                                    <label className="staff-label">Policy Title *</label>
                                    <input
                                        type="text"
                                        placeholder="e.g. Patient Bill Settlement & Refund Policy"
                                ...
                                <div className="form-row" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '16px' }}>
                                    <div className="form-group">
                                        ...
                                        <select
                                            ...
                                            className="staff-input"
                                        >
                                            {['General', 'Admission', 'Discharge', 'Billing & Refund', 'OPD Consultation', 'Emergency Care', 'Visitor & Attendant', 'Patient Rights & Responsibilities', 'Data Privacy & Consent', 'Pharmacy & Medication'].map(c => (
                                    ...
                                    <div className="form-group">
                                        <label className="staff-label">Applicable To</label>
                                        <select
                                ...
                                <div className="form-group" style={{ marginBottom: '16px' }}>
                                    <label className="staff-label">Policy Terms & Content *</label>
                                    <textarea
                                        rows={6}
                                        placeholder="Provide clear, comprehensive terms, guidelines, or legal consent clauses..."
                                ...
                                <div style={{ display: 'flex', gap: '20px', marginBottom: '20px', flexWrap: 'wrap' }}>
                                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '0.88rem', color: '#334155' }}>
                                        <input
                                            type="checkbox"
                                            checked={policyForm.isMandatory}
                                            onChange={(e) => setPolicyForm({ ...policyForm, isMandatory: e.target.checked })}
                                            style={{ accentColor: '#2563eb', width: '16px', height: '16px' }}
                                        />
                                        Mandatory agreement for registration & booking
                                    </label>
                                    ...
                                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '0.88rem', color: '#334155' }}>
                                        ...
                                        />
                                        Active (published to portals)
                                    </label>
                                </div>

                                <div className="modal-buttons" style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
                                    <button type="button" onClick={() => setPolicyModal({ open: false, mode: 'create', policy: null })} className="btn-cancel">
                                        Cancel
                                    </button>
                                    <button type="submit" disabled={savingPolicy} className="btn-save" style={{ background: '#2563eb', color: '#fff' }}>
                                        {savingPolicy ? 'Saving...' : (policyModal.mode === 'create' ? 'Create Policy' : 'Update Policy')}
                                    </button>
                                </div>
                            </form>
                        </div>
                ...
                {policyModal.open && policyModal.mode === 'preview' && policyModal.policy && (
                    <div className="modal-overlay">
                        <div data-lenis-prevent="true" className="modal-content ha-modal-modern" style={{ maxWidth: '640px' }}>
                            <div className="ha-modal-header">
                                <div className="ha-modal-header-icon blue">👁️</div>
                                <div>
                                    <h3 className="ha-modal-title">{policyModal.policy.title}</h3>
                                    <p className="ha-modal-subtitle">
                                        Category: {policyModal.policy.category} • Version v{policyModal.policy.version} • {policyModal.policy.isMandatory ? 'Mandatory' : 'Optional'}
                                    </p>
                                </div>
                                <button type="button" onClick={() => setPolicyModal({ open: false, mode: 'create', policy: null })} className="ha-modal-close-btn" title="Close">✕</button>
                            </div>
                            ...
                            <div style={{ padding: '24px', maxHeight: '60vh', overflowY: 'auto' }}>
                                <div style={{ background: '#f8fafc', padding: '18px', borderRadius: '10px', border: '1px solid #e2e8f0', whiteSpace: 'pre-wrap', lineHeight: 1.6, fontSize: '0.92rem', color: '#334155' }}>
                                    {policyModal.policy.content}
                                </div>
                            </div>

                            <div style={{ padding: '16px 24px', background: '#f1f5f9', display: 'flex', justifyContent: 'flex-end', borderRadius: '0 0 16px 16px' }}>
                                <button type="button" onClick={() => setPolicyModal({ open: false, mode: 'create', policy: null })} className="btn-cancel">
                                    Close Preview
                                </button>
                            </div>
                ...
                {emailPreviewModal.open && emailPreviewModal.preview && (
                    <div className="modal-overlay">
                        <div data-lenis-prevent="true" className="modal-content ha-modal-modern" style={{ maxWidth: '720px' }}>
                            <div className="ha-modal-header">
                                <div className="ha-modal-header-icon blue">✉️</div>
                                <div>
                                    <h3 className="ha-modal-title">
                                        {emailPreviewModal.type === 'LOGIN_OTP' ? 'Staff Login OTP Email Preview' : 'Appointment Confirmation Email Preview'}
                                    </h3>
                                    <p className="ha-modal-subtitle">
                                        Rendered with active hospital white-label identity.
                                    </p>
                                </div>
                                <button type="button" onClick={() => setEmailPreviewModal({ open: false, type: 'LOGIN_OTP', preview: null })} className="ha-modal-close-btn" title="Close">✕</button>
                            </div>

                            <div style={{ padding: '18px 24px', borderBottom: '1px solid #e2e8f0', background: '#f8fafc', fontSize: '0.85rem' }}>
                                <div style={{ marginBottom: '6px' }}>
                                    <strong style={{ color: '#475569' }}>From:</strong>{' '}
                                    <span style={{ color: '#0f172a', fontWeight: 600 }}>{emailPreviewModal.preview.from}</span>
                                </div>
                                <div style={{ marginBottom: '6px' }}>
                                    <strong style={{ color: '#475569' }}>To:</strong>{' '}
                                    <span style={{ color: '#0f172a' }}>{emailPreviewModal.preview.to}</span>
                                </div>
                                <div>
                                    <strong style={{ color: '#475569' }}>Subject:</strong>{' '}
                                    <span style={{ color: '#2563eb', fontWeight: 700 }}>{emailPreviewModal.preview.subject}</span>
                                </div>
                            </div>

                            <div style={{ padding: '20px 24px', maxHeight: '55vh', overflowY: 'auto', background: '#fff' }}>
                                <div
                                    style={{ border: '1px solid #e2e8f0', borderRadius: '8px', overflow: 'hidden' }}
                                    dangerouslySetInnerHTML={{ __html: emailPreviewModal.preview.html }}
                                />
                            </div>

                            <div style={{ padding: '16px 24px', background: '#f1f5f9', display: 'flex', justifyContent: 'flex-end', borderRadius: '0 0 16px 16px' }}>
                                <button type="button" onClick={() => setEmailPreviewModal({ open: false, type: 'LOGIN_OTP', preview: null })} className="btn-cancel">
                                    Close Preview
                                </button>
                            </div>
                        </div>
