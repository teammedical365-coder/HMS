...
// --- HELPER: Unified Permission Checker ---
// Allows access if user has:
// 1. A Legacy Role (e.g., 'reception')
// 2. OR A Dynamic Role with the required permission (e.g., 'appointment_manage')
// 3. OR Is a Super Admin
const checkAccess = (user, legacyRoles = [], requiredPermission = '') => {
    // 1. Super Admin Bypass
    if (user.role === 'administrator') return true;
    if (user.roles && user.roles.some(r => r.name === 'Super Admin' || r.name === 'Administrator')) return true;

    // 2. Legacy Role Check
    if (legacyRoles.includes(user.role)) return true;

    // 3. Custom Permission Override (User specific)
    if (user.customPermissions) {
        const custom = user.customPermissions.find(cp => cp.permission === requiredPermission);
        if (custom) return custom.granted;
    }

    // 4. Dynamic Role Permission Check
    if (user.roles && user.roles.some(r => r.permissions && r.permissions.includes(requiredPermission))) {
        return true;
    }
...
router.get('/reception/all', verifyToken, async (req, res) => {
    try {
        // Check: Legacy 'reception'/'admin' OR Permission 'appointment_view_all'
        if (!checkAccess(req.user, ['reception', 'admin'], 'appointment_view_all')) {
...
router.patch('/reception/reschedule/:id', verifyToken, async (req, res) => {
    try {
        // Check: Legacy 'reception'/'admin' OR Permission 'appointment_manage'
        if (!checkAccess(req.user, ['reception', 'admin'], 'appointment_manage')) {
...
router.post('/create', verifyToken, async (req, res) => {
    try {
        ...
        const appointment = new Appointment({
            ...
            // Fields that might be filled initially or later
            notes: notes || '',
